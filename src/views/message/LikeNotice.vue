<template>
  ...
  <div class="content__wrapper" v-loading="loading">
    ...
    <div v-for="item in likeNoticeList" :key="item.id" class="notice__item" @click="handleNoticeClick(item.noteId)">
      ...
      <div class="notice__wrapper">
        <div class="notice__middle">
          <div>
            ...
            <span class="notice__type" v-if="item.targetEntity.headerImg">赞了你的笔记</span>
            <span class="notice__type" v-else>赞了你的评论</span>
          </div>
    ...
    <span class="noMore" v-if="likeNoticeList.length">没有更多啦~</span>
  </div>
</template>
...
export default {
  ...
  setup () {
    ...
    const likeNoticeList = ref([]) // 点赞通知列表
    ...
    const getLikeNoticeList = async () => {
      try {
        ...
        if (result.code === 200 && result.data) {
          ...
          console.log('格式化时间后的点赞列表', list)
          likeNoticeList.value = [...list]
