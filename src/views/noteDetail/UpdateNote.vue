<template>
  ...
  <div class="header">
    ...
    <span style="font-size: .16rem;">重新编辑笔记</span>
    <!-- 发布按钮 -->
    <el-button class="post__button" type="primary" @click="updateNote">发布</el-button>
  </div>
  <!-- 笔记内容编辑区域 -->
  <div class="content">
    <el-input
...
export default {
  ...
  setup () {
    ...
    const getNoteData = async () => {
      try {
        const result = await get(`/note/getNoteForUpdate/${route.params.noteId}`)
        if (result.code === 200) {
          noteData.title = result.data.note.title
          noteData.content = result.data.note.content
          noteData.type = result.data.note.type === 0 ? '美食笔记' : '探店笔记'
          // 处理获取到的标签
          result.data.tags.forEach(tagIndex => {
            if (tagIndex >= 0 && tagIndex <= 5) {
              noteData.tagsName[tagIndex] = tagList.value.localDishes[tagIndex]
            } else if (tagIndex >= 6 && tagIndex <= 9) {
              noteData.tagsName[tagIndex] = tagList.value.foreignDishes[tagIndex]
            } else {
              noteData.tagsName[tagIndex] = tagList.value.otherDishes[tagIndex]
            }
          })
          // 处理获取到的图片组
          result.data.images.forEach(image => {
            noteData.photoList.push({ url: image })
          })
        } else {
          ElMessage({
            showClose: true,
    ...
    const updateNote = async () => {
      try {
        ...
        formData.append('id', route.params.noteId)
        ...
        const result = await post('/note/updatePost', formData)
        if (result.code === 200) {
